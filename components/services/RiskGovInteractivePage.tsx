            ...
            {internalServices.map((item, idx) => (
              <AnimatedSection key={idx} delay={idx * 0.04}>
                <motion.div whileHover={{ y: -3, scale: 1.01 }} transition={{ duration: 0.2 }}>
                  <Link
                    href={item.href}
                    className="p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-[#fefaf5] dark:bg-[#0a0a0a] border border-[#f7d7b0] dark:border-[#1a1a1a] hover:border-[#2e936f] hover:shadow-md transition-all flex items-center justify-between group cursor-pointer min-h-[60px]"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-xl bg-white dark:bg-[#000000] border border-[#f7d7b0] dark:border-[#1a1a1a] group-hover:scale-105 group-hover:border-[#2e936f]/40 transition-all shrink-0">
                        {item.icon}
                      </div>
